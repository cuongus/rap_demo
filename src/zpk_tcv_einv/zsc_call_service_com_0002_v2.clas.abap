CLASS zsc_call_service_com_0002_v2 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .

  PUBLIC SECTION.
    TYPES: tt_header TYPE TABLE OF zrap_einv_entry,
    ...
    CLASS-METHODS:
      "Create Contructor
      get_Instance RETURNING VALUE(ro_instance) TYPE REF TO zsc_call_service_com_0002_v2,

      change_journal_entry_http IMPORTING i_header             TYPE tt_header
                                          i_accountingdocument TYPE tt_accountingdocument OPTIONAL
                                RAISING
                                          cx_uuid_error
  ...
  PROTECTED SECTION.
  PRIVATE SECTION.
    CLASS-DATA: mo_instance TYPE REF TO zsc_call_service_com_0002_v2.
ENDCLASS.



CLASS ZSC_CALL_SERVICE_COM_0002_V2 IMPLEMENTATION.
...
*          lv_uuid TYPE string VALUE `urn:uuid:{{$randomUUID}}`.
    ...
    IF sy-subrc EQ 0.
      SHIFT ls_header-seq LEFT DELETING LEADING '0'.
      lv_seq   = |1{ ls_header-serial }#{ ls_header-seq }|.
      lv_bukrs = ls_header-companycode.
      lv_belnr = ls_header-accountingdocument.
      lv_gjahr = ls_header-fiscalyear.
      lv_date_ph = |{ ls_header-dateiss+0(4) }-{ ls_header-dateiss+4(2) }-{ ls_header-dateiss+6(2) }|.
...
*      AND Customer = @ls_header-customer
*    INTO @DATA(ls_accountingdocumentitem).
*    IF sy-subrc EQ 0.
*        lv_buzei = ls_accountingdocumentitem-AccountingDocumentItem.
*    ENDIF.
    LOOP AT i_accountingdocument INTO DATA(ls_accountingdocument).
      TRY.

          lv_buzei = ls_accountingdocument-AccountingDocumentItem.
...
*-- SET HTTP Header Fields
          ...
          lv_password = |Abcd@1234567890Efghijk|.
